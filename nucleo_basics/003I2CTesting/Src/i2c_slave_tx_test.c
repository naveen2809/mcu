...
int main(void)
{
	...
	char message[]="Hello World message from STM32F446RE Nucleo Board!!!!!!\r\n";
	char message2[]="Second message from same board!\r\n";
	...
	while(1)
	{
		...
		I2CSlaveSendData(I2C1,MessageLen,2);
		I2CSlaveSendData(I2C1,(uint8_t *)message,strlen(message));
		ToggleLED();
		MessageLen[0] = (uint8_t) strlen(message2);
		MessageLen[1] = (uint8_t) strlen(message2);
		I2CSlaveSendData(I2C1,MessageLen,2);
		I2CSlaveSendData(I2C1,(uint8_t *)message2,strlen(message2));
		ToggleLED();
	}
