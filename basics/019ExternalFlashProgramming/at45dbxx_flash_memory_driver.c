...
uint8_t at45dbxx_block_erase(uint8_t *PageAddress)
{
	...
	//Check status of Block Erase operation
	//Step (a): Wait for the operation to complete
	do
	{
		at45dbxx_read_status_register(status);
...
uint8_t at45dbxx_sector_erase(uint8_t *PageAddress)
{
	...
	//Check status of Sector Erase operation
	//Step (a): Wait for the operation to complete
	do
	{
		at45dbxx_read_status_register(status);
...
uint8_t at45dbxx_chip_erase(void)
{
	...
	//Check status of Chip Erase operation
	//Step (a): Wait for the operation to complete
	do
	{
		at45dbxx_read_status_register(status);
...
uint8_t at45dbxx_page_to_buffer_compare(uint8_t Buffer, uint8_t *PageAddress)
{
	uint32_t CommandLength;
	uint8_t *tx_buffer;
	uint8_t status[2];
	...
	if((status[0] & AT45DBXX_STATUS_COMPARE_BITMASK) == 0)
	{
		return AT45DBXX_BUFFER_AND_PAGE_EQUAL;
	}
	else
	{
		return AT45DBXX_BUFFER_AND_PAGE_NOT_EQUAL;
	}
}
...
{
	if(status == AT45DBXX_SUCCESS)
	{
		printf("Operation Successful\r\n");
	}
	else if(status == AT45DBXX_EPE_ERROR)
	{
		printf("Operation Failed Due to Erase/Program Error\r\n");
		while(1);
	}
	else if(status == AT45DBXX_FAIL)
	{
		printf("Operation Failed Due to Memory Allocation Error\r\n");
		while(1);
	}
}
