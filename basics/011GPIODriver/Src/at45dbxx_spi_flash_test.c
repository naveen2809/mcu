...
/*
	...
	at45dbxx_page_to_buffer_transfer(MemAddress,AT45DBXX_SRAM_BUFFER_1);
	GPIOWritePin(FLASH_PORT,TEST_PIN,GPIO_LOW);
...
/*
	...
	//Write Page to Flash
	GPIOWritePin(FLASH_PORT,TEST_PIN,GPIO_HIGH);
	status = at45dbxx_read_modify_write(AT45DBXX_SRAM_BUFFER_1,MemAddress,tx_buffer,10);
	GPIOWritePin(FLASH_PORT,TEST_PIN,GPIO_LOW);
	printf("Status of Page Program without Erase Operation: ");
	error_handler(status);

	status = at45dbxx_page_to_buffer_compare(AT45DBXX_SRAM_BUFFER_1,MemAddress);
	if(status == AT45DBXX_BUFFER_AND_PAGE_EQUAL)
	{
		printf("PASS: Buffer and Page Contents are the Same\r\n");
	}
	else if(status == AT45DBXX_BUFFER_AND_PAGE_NOT_EQUAL)
	{
		printf("FAIL: Buffer and Page Contents are NOT the Same\r\n");
	}
