...
int main(void)
{
	...
	GPIOWritePin(GPIOD, GPIO_PIN_LED_1, GPIO_LOW);
	GPIOWritePin(GPIOD, GPIO_PIN_LED_2, GPIO_LOW);
	GPIOWritePin(GPIOD, GPIO_PIN_LED_3, GPIO_LOW);
	GPIOWritePin(GPIOD, GPIO_PIN_LED_4, GPIO_LOW);

	delay();

	GPIOWritePin(GPIOD, GPIO_PIN_LED_1, GPIO_HIGH);
	GPIOWritePin(GPIOD, GPIO_PIN_LED_2, GPIO_HIGH);
	GPIOWritePin(GPIOD, GPIO_PIN_LED_3, GPIO_HIGH);
	GPIOWritePin(GPIOD, GPIO_PIN_LED_4, GPIO_HIGH);
	...
	//TIM6_Handle.Basic_Timer_Config.Timer_OnePulseModeConfig = TIMER_ONE_PULSE_MODE_ENABLE;

	EnablePeriClk(TIM6);
	Basic_Timer_Init(&TIM6_Handle);
	//Basic_Timer_SetARPEFlag(&TIM6_Handle);
	...
	//TIM7_Handle.Basic_Timer_Config.Timer_OnePulseModeConfig = TIMER_ONE_PULSE_MODE_ENABLE;

	EnablePeriClk(TIM7);
	Basic_Timer_Init(&TIM7_Handle);
	//Basic_Timer_SetARPEFlag(&TIM7_Handle);
...
void Basic_Timer_Interrupt_Callback(struct Basic_Timer_Handle_t *pBasic_Timer_Handle)
{
	...
	Basic_Timer_PeripheralDisable(pBasic_Timer_Handle);
