...
int main(void)
{
  ...
  status = xTaskCreate(task_orange_led,"orange_led_task",1024,NULL,2,&task_handle_orange_led);
  configASSERT(status == pdPASS);

  status = xTaskCreate(task_red_led,"red_led_task",1024,NULL,3,&task_handle_red_led);
  configASSERT(status == pdPASS);

  status = xTaskCreate(task_blue_led,"blue_led_task",1024,NULL,3,&task_handle_blue_led);
  configASSERT(status == pdPASS)
...
static void task_orange_led(void *params)
{
    while(1)
    {
    	HAL_GPIO_TogglePin(LED_GPIO_PORT,GPIO_ORANGE_LED);
        delay(500000);
    }
}
...
static void configure_led_gpios(void)
{
	...
	GPIO_InitStruct.Pin = GPIO_GREEN_LED|GPIO_ORANGE_LED|GPIO_RED_LED|GPIO_BLUE_LED;
	GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
	GPIO_InitStruct.Pull = GPIO_NOPULL;
	GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
	HAL_GPIO_Init(GPIOD, &GPIO_InitStruct);
